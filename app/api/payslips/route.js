...
async function getEmployee(personnelId) {
  const result = await pool.query(`SELECT id, full_name, personnel_code, national_id, department, job_title, bank_account, job_group FROM personnel WHERE id=$1`, [personnelId]);
...
export async function PUT(request) {
  ...
  try {
    const body = await request.json();
    const payslipId = Number(body.id), personnelId = Number(body.personnel_id);
    if (!payslipId) return NextResponse.json({ success:false, error:"شناسه فیش مشخص نشده است." }, { status:400 });
    if (!personnelId) return NextResponse.json({ success:false, error:"لطفاً کارمند را انتخاب کنید." }, { status:400 });
    const existing = await pool.query("SELECT id FROM payslips WHERE id=$1", [payslipId]);
