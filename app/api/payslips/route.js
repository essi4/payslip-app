...
function prepareCalculation(body) { const monthNumber = normalizeMonth(body.month); if (!monthNumber) return { error: "ماه حقوق نامعتبر است." }; if (Number(body.year || 1405) !== 1405) return { error: "موتور محاسبه این نسخه فعلاً مخصوص سال ۱۴۰۵ است." }; const calc = calculatePayroll1405({ ...body, year: 1405, monthNumber }); if (calc.baseSalary <= 0) return { error: "حقوق پایه باید بیشتر از صفر باشد." }; return { calc }; }
...
export async function PUT(request) {
  ...
  try {
    ...
    const year = String(body.year ?? "1405").trim(), month = String(body.month ?? "فروردین").trim(); const period = await getPeriod(employee.company_id, year, month); if (!period) return invalidPeriodResponse(); if (period.status === "closed") return closedPeriodResponse();
    const prepared = prepareCalculation(body); if (prepared.error) return NextResponse.json({ success: false, error: prepared.error }, { status: 400 }); const c = prepared.calc;
