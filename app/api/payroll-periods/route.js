...
export async function POST(request) {
  try {
    ...
    if (!Number.isInteger(companyId) || !Number.isInteger(year) || !Number.isInteger(month)) {
      return NextResponse.json(
        { success: false, error: "company_id، year و month الزامی هستند" },
        { status: 400 }
      );
    }
    ...
    if (month < 1 || month > 12) {
      return NextResponse.json(
        { success: false, error: "ماه باید بین ۱ تا ۱۲ باشد" },
        { status: 400 }
      );
    }
    ...
    if (duplicate.rowCount > 0) {
      return NextResponse.json(
        { success: false, error: "این دوره برای این شرکت قبلاً ثبت شده است" },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: "خطا در ایجاد دوره حقوق" },
      { status: 500 }
    );
  }
}
