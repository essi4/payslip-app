    ...
    return NextResponse.json({
      ...
      data: {
        employeesCount: Number(
          employeesResult.rows[0]?.employees_count || 0
        ),

        payslipsCount: Number(
          summary?.payslips_count || 0
        ),

        totalBaseSalary: Number(
          summary?.total_base_salary || 0
        ),

        totalBenefits: Number(
          summary?.total_benefits || 0
        ),
        ...
        totalNetSalary: Number(
          summary?.total_net_salary || 0
        ),

        monthlyReports:
          monthlyResult.rows.map((item) => ({
            year: item.year,
            month: item.month,

            payslipsCount: Number(
              item.payslips_count || 0
            ),
