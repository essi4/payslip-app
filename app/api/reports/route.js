  ...
  try {
    ...
    const summaryResult = await pool.query(`
      SELECT COUNT(*)::int AS payslips_count,
        ...
        COALESCE(SUM(COALESCE(p.overtime,0)+COALESCE(p.bonus,0)+COALESCE(p.housing_allowance,0)+COALESCE(p.food_allowance,0)+COALESCE(p.marriage_allowance,0)+COALESCE(p.child_allowance,0)+COALESCE(p.other_benefits,0)),0) AS total_benefits,
        COALESCE(SUM(COALESCE(p.insurance,0)+COALESCE(p.tax,0)+COALESCE(p.other_deductions,0)),0) AS total_deductions,
        COALESCE(SUM(p.net_salary),0) AS total_net_salary
      FROM payslips p LEFT JOIN personnel e ON e.id=p.personnel_id ${where}
    `, values);
    ...
    const monthlyResult = await pool.query(`
      SELECT p.year,p.month,COUNT(*)::int AS payslips_count,
        COALESCE(SUM(p.base_salary),0) AS base_salary,
        COALESCE(SUM(COALESCE(p.overtime,0)+COALESCE(p.bonus,0)+COALESCE(p.housing_allowance,0)+COALESCE(p.food_allowance,0)+COALESCE(p.marriage_allowance,0)+COALESCE(p.child_allowance,0)+COALESCE(p.other_benefits,0)),0) AS benefits,
        COALESCE(SUM(COALESCE(p.insurance,0)+COALESCE(p.tax,0)+COALESCE(p.other_deductions,0)),0) AS deductions,
        COALESCE(SUM(p.net_salary),0) AS net_salary
      FROM payslips p LEFT JOIN personnel e ON e.id=p.personnel_id ${where}
      GROUP BY p.year,p.month ORDER BY p.year DESC,MIN(p.id) DESC
    `, values);

    const payslipsResult = await pool.query(`
      SELECT p.id,p.personnel_id,p.year,p.month,p.base_salary,
        COALESCE(p.overtime,0)+COALESCE(p.bonus,0)+COALESCE(p.housing_allowance,0)+COALESCE(p.food_allowance,0)+COALESCE(p.marriage_allowance,0)+COALESCE(p.child_allowance,0)+COALESCE(p.other_benefits,0) AS benefits,
        COALESCE(p.insurance,0)+COALESCE(p.tax,0)+COALESCE(p.other_deductions,0) AS deductions,
        p.net_salary,p.created_at,e.full_name,e.personnel_code,e.company_id,c.name AS company_name
      FROM payslips p LEFT JOIN personnel e ON e.id=p.personnel_id LEFT JOIN companies c ON c.id=e.company_id
