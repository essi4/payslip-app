...
export async function GET(request) {
  try {
    ...
    if (!nationalId) {
      return NextResponse.json({ success: false, error: "کد ملی وارد نشده است." }, { status: 400 });
    }

    if (!password) {
      return NextResponse.json({ success: false, error: "رمز عبور وارد نشده است." }, { status: 400 });
    }

    const cleanNationalId = nationalId.replace(/[^0-9]/g, "");

    if (cleanNationalId.length !== 10) {
      return NextResponse.json({ success: false, error: "کد ملی باید ۱۰ رقمی باشد." }, { status: 400 });
    }

    const employeeResult = await pool.query(
      `SELECT id, full_name, national_id, personnel_code, department
       FROM personnel
       WHERE national_id = $1 AND payslip_password = $2
       LIMIT 1`,
    ...
    if (employeeResult.rows.length === 0) {
      return NextResponse.json(
        { success: false, error: "کد ملی یا رمز عبور اشتباه است." },
        { status: 401 }
      );
    }
    ...
    if (!month || !year) {
      const monthsResult = await pool.query(
        `SELECT id, year, month, net_salary, created_at
         FROM payslips
    ...
    if (payslipResult.rows.length === 0) {
      return NextResponse.json(
        { success: false, error: "برای این ماه فیشی پیدا نشد." },
        { status: 404 }
      );
    }
    ...
    // Do not expose database host, connection strings, SQL details, or stack traces.
    return NextResponse.json(
      { success: false, error: "خطا در ارتباط با سامانه. لطفاً دوباره تلاش کنید." },
      { status: 500 }
    );
  }
}
