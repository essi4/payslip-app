...
export async function POST(request) {
  try {
    ...
    if (!session) {
      await logSecurityEvent({ event: "password_change", request, success: false, details: { reason: "no_session" } });
      return NextResponse.json({ success: false, error: "نشست شما منقضی شده است. دوباره وارد شوید." }, { status: 401 });
    }
    ...
    if (!currentPassword || !newPassword || !confirmPassword) return NextResponse.json({ success: false, error: "همه فیلدهای رمز عبور را کامل کنید." }, { status: 400 });
    if (newPassword.length < 8) return NextResponse.json({ success: false, error: "رمز عبور جدید باید حداقل ۸ کاراکتر باشد." }, { status: 400 });
    if (newPassword !== confirmPassword) return NextResponse.json({ success: false, error: "رمز عبور جدید و تکرار آن یکسان نیست." }, { status: 400 });
    if (newPassword === currentPassword) return NextResponse.json({ success: false, error: "رمز عبور جدید باید با رمز فعلی متفاوت باشد." }, { status: 400 });
    ...
    if (!check.valid) {
      await logSecurityEvent({ employeeId: session.employeeId, event: "password_change", request, success: false, details: { reason: "invalid_current_password" } });
      return NextResponse.json({ success: false, error: "رمز عبور فعلی صحیح نیست." }, { status: 400 });
    }

    const passwordHash = await hashPassword(newPassword);
    await pool.query("UPDATE personnel SET payslip_password=$1 WHERE id=$2", [passwordHash, session.employeeId]);
    await logSecurityEvent({ employeeId: session.employeeId, event: "password_change", request, success: true });
