...
export async function POST(request) {
  try {
    ...
    catch (emailError) { console.error("PASSWORD RECOVERY EMAIL ERROR:", emailError); await pool.query(`UPDATE personnel SET password_reset_code=NULL, password_reset_expires_at=NULL WHERE id=$1`, [employee.id]); await logSecurityEvent({ employeeId: employee.id, event: "password_recovery_request", request, success: false, details: { reason: "email_failed" } }); return NextResponse.json({ success: false, error: "ارسال ایمیل بازیابی انجام نشد." }, { status: 500 }); }
