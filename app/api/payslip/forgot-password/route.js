...
function rateLimitedResponse(retryAfterSeconds) {
  return NextResponse.json(
    { success: false, error: "تعداد درخواست‌های بازیابی بیش از حد مجاز است. لطفاً کمی بعد دوباره تلاش کنید." },
...
export async function POST(request) {
  try {
    ...
    const ipLimit = await checkRateLimit({ scope: "password_recovery_ip", key: requestIp, maxAttempts: 5, windowMs: 60 * 60 * 1000, blockMs: 60 * 60 * 1000 });
    if (!ipLimit.allowed) {
      await logSecurityEvent({ event: "password_recovery_rate_limit", request, success: false, details: { reason: "ip_limit" } });
    ...
    if (!result.rows.length || !result.rows[0].email) {
      await logSecurityEvent({ event: "password_recovery_request", request, success: true, details: { matched_account: false } });
    ...
    const accountLimit = await checkRateLimit({ scope: "password_recovery_account", key: `${nationalId}:${personnelCode}`, maxAttempts: 3, windowMs: 60 * 60 * 1000, blockMs: 60 * 60 * 1000 });
    if (!accountLimit.allowed) {
      await logSecurityEvent({ employeeId: employee.id, event: "password_recovery_rate_limit", request, success: false, details: { reason: "account_limit" } });
      return rateLimitedResponse(accountLimit.retryAfterSeconds);
    }

    if (employee.password_reset_code && employee.password_reset_expires_at && new Date(employee.password_reset_expires_at).getTime() > Date.now()) {
      await logSecurityEvent({ employeeId: employee.id, event: "password_recovery_request", request, success: false, details: { reason: "cooldown" } });
      return NextResponse.json({ success: false, error: "یک کد بازیابی قبلاً ارسال شده است. لطفاً تا پایان اعتبار آن صبر کنید." }, { status: 429 });
    }
    ...
    try {
      ...
      await logSecurityEvent({ employeeId: employee.id, event: "password_recovery_request", request, success: false, details: { reason: "email_failed" } });
      return NextResponse.json({ success: false, error: "ارسال ایمیل بازیابی انجام نشد." }, { status: 500 });
    }

    await logSecurityEvent({ employeeId: employee.id, event: "password_recovery_request", request, success: true });
    return NextResponse.json({ success: true, message: "کد بازیابی به ایمیل ثبت‌شده ارسال شد." });
  } catch (error) {
    console.error("FORGOT PASSWORD ERROR:", error);
    return NextResponse.json({ success: false, error: "خطا در بازیابی رمز عبور. دوباره تلاش کنید." }, { status: 500 });
  }
}
