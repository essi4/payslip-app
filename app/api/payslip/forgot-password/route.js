...
export async function POST(request) {
  try {
    ...
    if (!nationalId) {
      return NextResponse.json({ success: false, error: "کد ملی را وارد کنید." }, { status: 400 });
    }
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ success: false, error: "کارمندی با این کد ملی پیدا نشد." }, { status: 404 });
    }
