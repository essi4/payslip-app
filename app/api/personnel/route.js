...
export async function GET(request) {
  ...
  try {
    const result = await pool.query(`
      SELECT p.id, p.full_name, p.national_id, p.personnel_code, p.department,
        p.job_title, p.bank_account, p.job_group, p.company_id, p.created_at,
...
export async function POST(request) {
  ...
  try {
    ...
    const result = await pool.query(`
      INSERT INTO personnel (company_id, full_name, national_id, personnel_code,
        department, job_title, bank_account, job_group, payslip_password)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
      RETURNING id, full_name, national_id, personnel_code, department, job_title,
        bank_account, job_group, company_id, created_at
    `, [
      companyId, fullName, nationalId, personnelCode,
      body.department?.trim() || null, body.job_title?.trim() || null,
      body.bank_account?.trim() || null, body.job_group?.trim() || null, passwordHash,
    ]);

    return Response.json({ success: true, data: result.rows[0] });
  } catch (error) {
    console.error("Personnel POST error:", error);
    return Response.json({ success: false, error: error?.message || "خطا در ثبت کارمند" }, { status: 500 });
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    const result = await pool.query(`
      UPDATE personnel SET company_id=$1, full_name=$2, national_id=$3, personnel_code=$4,
        department=$5, job_title=$6, bank_account=$7, job_group=$8, payslip_password=$9
      WHERE id=$10
      RETURNING id, full_name, national_id, personnel_code, department, job_title,
        bank_account, job_group, company_id, created_at
    `, [
      companyId, String(body.full_name || "").trim(), nationalId,
      String(body.personnel_code || "").trim(), body.department?.trim() || null,
      body.job_title?.trim() || null, body.bank_account?.trim() || null,
      body.job_group?.trim() || null, passwordHash, id,
    ]);
    ...
    return Response.json({ success: false, error: error?.message || "خطا در ویرایش کارمند" }, { status: 500 });
  }
}
...
export async function DELETE(request) {
  ...
  try {
    ...
    return Response.json({ success: false, error: error?.message || "خطا در حذف کارمند" }, { status: 500 });
  }
}
