...
export default function ForgotPasswordPage() {
  ...
  async function requestReset() {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setError(
          result.error || "خطا در ایجاد کد بازیابی."
        );
        return;
      }

      setServerCode(String(result.code || ""));

      setMessage(
        "کد بازیابی با موفقیت ایجاد شد."
      );
    } catch (error) {
      console.error(error);
  ...
  function verifyCode() {
    ...
    setMessage(
      "کد صحیح است. اکنون رمز جدید خود را تعیین کنید."
    );
  }
  ...
  async function resetPassword() {
    ...
    if (newPassword !== confirmPassword) {
      setError(
        "رمز جدید و تکرار رمز یکسان نیستند."
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setError(
          result.error || "خطا در تغییر رمز عبور."
        );
        return;
      }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {serverCode && !codeVerified && (
            <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4 text-center">
              ...
              <div
                dir="ltr"
                className="mt-2 text-2xl font-black tracking-[6px] text-slate-900"
              >
                {serverCode}
              </div>

              <div className="mt-2 text-[10px] text-slate-500">
                این کد فعلاً برای تست نمایش داده می‌شود.
              </div>
          ...
          {serverCode && !codeVerified && (
            <div className="mt-5 border-t border-slate-100 pt-5">
              ...
              <button
                type="button"
                onClick={verifyCode}
                className="mt-3 w-full rounded-xl bg-blue-600 px-4 py-3 text-xs font-black text-white transition hover:bg-blue-700"
              >
                تأیید کد
              </button>
          ...
          {codeVerified && (
            <div className="border-t border-slate-100 pt-2">
              ...
              <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-bold leading-6 text-emerald-700">
                کد صحیح است. رمز جدید خود را وارد کنید.
              </div>
              ...
              <label className="mb-2 mt-4 block text-xs font-bold text-slate-700">
                تکرار رمز عبور جدید
              </label>
              ...
              <button
                ...
                disabled={loading}
                className="mt-4 w-full rounded-xl bg-emerald-600 px-4 py-3 text-xs font-black text-white transition hover:bg-emerald-700 disabled:opacity-50"
              >
                {loading
                  ? "در حال ذخیره..."
                  : "ذخیره رمز جدید"}
