  ...
  useEffect(() => {
    ...
    fetch("/api/payslip", {
      ...
      .then((data) => {
        ...
        setPayslipCount(Array.isArray(data.months) ? data.months.length : 0);
      })
      .catch(() => {});
  ...
  const cards = [
    {
      ...
      icon: "📄",
      title: "فیش حقوقی",
      text: `${payslipCount.toLocaleString("fa-IR")} فیش ثبت شده · مشاهده و چاپ`,
      tone: "blue",
    },
    {
      href: "/payslip/order",
      icon: "📋",
      title: "حکم کارگزینی",
      text: "مشاهده اطلاعات حکم و مشخصات شغلی",
      tone: "violet",
    },
    {
      ...
      icon: "👤",
      title: "مشخصات من",
      text: "اطلاعات هویتی، شغلی و سازمانی",
      tone: "emerald",
    },
  ];
    ...
    <main dir="rtl" className="min-h-screen bg-slate-50 px-4 pb-28 pt-28 sm:px-6 sm:pt-32">
      <div className="mx-auto max-w-5xl">
        <section className="mb-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          {employee?.full_name && (
            <p className="mt-2 text-xs font-bold text-slate-500">برای {employee.full_name}</p>
          )}
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {cards.map((card) => (
            <Link
              key={card.href}
              href={card.href}
              className={`group min-h-[180px] rounded-[26px] border p-5 text-right shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-xl ${
                card.tone === "blue"
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-2xl shadow-sm ring-1 ring-black/5">
                  {card.icon}
                </div>
                <span className="rounded-full bg-white/80 px-3 py-1 text-[9px] font-black text-slate-600">
                  مشاهده ←
                </span>
              </div>
