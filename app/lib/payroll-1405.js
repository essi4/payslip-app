...
/** Unified 1405 payroll calculation. All monetary values are ریال. */
export function calculatePayroll1405(input = {}) {
  ...
  return {
    year, month, monthDays, workDays,
    baseSalary: roundPayroll(baseSalary), dailyWage: roundPayroll(dailyWage), hourlyWage: roundPayroll(hourlyWage),
    missionDays, missionHours, missionAllowance: roundPayroll(missionAllowance),
    seniorityEligible, seniorityDailyRate: roundPayroll(seniorityDailyRate), seniorityAllowance: roundPayroll(seniorityAllowance),
    overtimeHours, overtime: roundPayroll(overtime), bonus: roundPayroll(bonus),
    housingAllowance: roundPayroll(housingAllowance), foodAllowance: roundPayroll(foodAllowance), marriageAllowance: roundPayroll(marriageAllowance), childAllowance: roundPayroll(childAllowance), otherBenefits: roundPayroll(otherBenefits),
    insuranceBase: roundPayroll(insuranceBase), insurance: roundPayroll(insurance), taxableGross: roundPayroll(taxableGross), taxableIncome: roundPayroll(taxableIncome), tax: roundPayroll(tax), otherDeductions: roundPayroll(otherDeductions),
    grossSalary: roundPayroll(grossSalary), totalBenefits: roundPayroll(totalBenefits), totalDeductions: roundPayroll(totalDeductions), netSalary: roundPayroll(netSalary),
  };
}
