...
export default function PayslipView({ params }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const payslipResponse = await fetch("/api/payslips", { cache: "no-store" });
        const payslipResult = await payslipResponse.json();
        if (payslipResult.success && Array.isArray(payslipResult.data)) {
          const found = payslipResult.data.find((item) => String(item.id) === String(params.id));
      ...
      <div className="payslip-paper">
        <div className="company-header">
          <div className="company-title">
            {showCompanyName && <h1>{companyName}</h1>}
            <h2>{systemTitle}</h2>
            <p>فیش حقوقی</p>
          </div>
        ...
        <div className="period">
          <div><span>سال</span><strong>{payslip.year || "---"}</strong></div>
          <div><span>ماه</span><strong>{payslip.month || "---"}</strong></div>
          <div><span>وضعیت</span><strong className="approved">صادر شده</strong></div>
        </div>

        <div className="employee-info">
          <h2>👤 اطلاعات کارمند</h2>
          <div className="info-grid">
            <div><span>نام و نام خانوادگی</span><strong>{employeeName}</strong></div>
            <div><span>کد پرسنلی</span><strong>{personnelCode}</strong></div>
            {showBankAccount && <div><span>شماره حساب</span><strong>{payslip.bank_account || "---"}</strong></div>}
            {showJobGroup && <div><span>گروه شغلی</span><strong>{payslip.job_group || "---"}</strong></div>}
            {showJobTitle && <div><span>عنوان شغلی</span><strong>{payslip.job_title || "---"}</strong></div>}
          </div>
        </div>

        <div className="salary-section">
          <h2>💰 جزئیات حقوق و مزایا</h2>
          <table>
            ...
            <tbody>
              ...
              <tr><td>سنوات</td><td>{money(seniority)}</td></tr>
              <tr><td>ماموریت</td><td>{money(mission)}</td></tr>
        ...
        <div className="footer">
          <div className="issue-date"><span>تاریخ صدور</span><strong>{formatDate(payslip.created_at)}</strong></div>
          <div className="signature"><span>مسئول تأیید فیش</span>{managerName && <strong>{managerName}</strong>}{managerPosition && <small>{managerPosition}</small>}<div className="signature-line"></div><span>مهر و امضا</span></div>
        </div>
