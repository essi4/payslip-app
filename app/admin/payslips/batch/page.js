...
const CURRENCY = "ریال";
const emptyRow = (employee) => ({ personnel_id: employee.id, base_salary: "", work_days: "30", mission_days: "0", mission_hours: "0", seniority_eligible: false, overtime_hours: "0", bonus: "0", married: false, child_count: "0", other_benefits: "0", other_deductions: "0" });
...
export default function BatchPayslipsPage() {
  ...
  const selectedRows = useMemo(() => rows.filter((row) => Number(row.base_salary) > 0), [rows]);
  const totalBase = selectedRows.reduce((sum, row) => sum + Number(row.base_salary || 0), 0);

  function updateRow(id, field, value) {
    setRows((current) => current.map((row) => Number(row.personnel_id) === Number(id) ? { ...row, [field]: value } : row));
  }

  function setAllBase(value) {
    setRows((current) => current.map((row) => ({ ...row, base_salary: value })));
  }

  async function runPreview() {
    if (!companyId || !selectedRows.length) { setError("حداقل یک کارمند با حقوق پایه معتبر انتخاب کنید."); return; }
  ...
  return (
    <main dir="rtl" style={{ maxWidth: 1400, margin: "0 auto", padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap", marginBottom: 20 }}>
        <div><h1 style={{ margin: 0 }}>صدور گروهی فیش حقوقی ۱۴۰۵</h1><p style={{ color: "#64748b" }}>محاسبه یکپارچه پایه، سنوات، مأموریت، اضافه‌کاری، بیمه ۷٪، مالیات پلکانی و خالص پرداختی</p></div>
      ...
      <section style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: 18, marginBottom: 18 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 14 }}>
          ...
          <label>حقوق پایه یکسان برای همه (اختیاری)<input placeholder="مثلاً 188760879" onChange={(e) => setAllBase(e.target.value)} style={{ width: "100%", padding: 10, marginTop: 6 }} /><small>{CURRENCY}</small></label>
        </div>
      </section>

      <section style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1100 }}>
          <thead><tr>{["انتخاب/کارمند", "حقوق پایه", "کارکرد", "سنوات", "ماموریت روز", "ماموریت ساعت", "اضافه‌کاری ساعت", "تأهل", "فرزند", "پاداش", "سایر مزایا", "سایر کسورات"].map((h) => <th key={h} style={{ padding: 10, background: "#f8fafc", textAlign: "right", borderBottom: "1px solid #e2e8f0" }}>{h}</th>)}</tr></thead>
          <tbody>{employees.map((employee) => {
            ...
            return <tr key={employee.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
              <td style={{ padding: 10, whiteSpace: "nowrap" }}><label><input type="checkbox" checked={Number(row.base_salary) > 0} onChange={(e) => updateRow(employee.id, "base_salary", e.target.checked ? (row.base_salary || "1") : "")} /> {employee.full_name}<small style={{ display: "block", color: "#64748b" }}>{employee.personnel_code || ""}</small></label></td>
              <td><input value={row.base_salary} onChange={(e) => updateRow(employee.id, "base_salary", e.target.value)} style={{ width: 130, padding: 7 }} /></td>
              <td><input type="number" min="0" max="31" value={row.work_days} onChange={(e) => updateRow(employee.id, "work_days", e.target.value)} style={{ width: 65, padding: 7 }} /></td>
              ...
              <td><input type="number" min="0" value={row.mission_days} onChange={(e) => updateRow(employee.id, "mission_days", e.target.value)} style={{ width: 65, padding: 7 }} /></td>
              <td><input type="number" min="0" value={row.mission_hours} onChange={(e) => updateRow(employee.id, "mission_hours", e.target.value)} style={{ width: 65, padding: 7 }} /></td>
              <td><input type="number" min="0" value={row.overtime_hours} onChange={(e) => updateRow(employee.id, "overtime_hours", e.target.value)} style={{ width: 80, padding: 7 }} /></td>
              <td><input type="checkbox" checked={row.married} onChange={(e) => updateRow(employee.id, "married", e.target.checked)} /></td>
              <td><input type="number" min="0" value={row.child_count} onChange={(e) => updateRow(employee.id, "child_count", e.target.value)} style={{ width: 55, padding: 7 }} /></td>
              <td><input value={row.bonus} onChange={(e) => updateRow(employee.id, "bonus", e.target.value)} style={{ width: 90, padding: 7 }} /></td>
              <td><input value={row.other_benefits} onChange={(e) => updateRow(employee.id, "other_benefits", e.target.value)} style={{ width: 90, padding: 7 }} /></td>
              <td><input value={row.other_deductions} onChange={(e) => updateRow(employee.id, "other_deductions", e.target.value)} style={{ width: 90, padding: 7 }} /></td>
            </tr>;
          })}</tbody>
        </table>
      </section>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginTop: 18 }}>
        ...
        <span>کارکنان انتخاب‌شده: <b>{selectedRows.length.toLocaleString("fa-IR")}</b> — جمع پایه: <b>{money(totalBase)} {CURRENCY}</b></span>
      </div>
