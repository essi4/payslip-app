  ...
  return <article className="payslip-sheet rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-8">
    <header className="border-b-2 border-slate-900 pb-4">
      <div className="flex items-start justify-between gap-4">
        <div><div className="text-xs font-bold text-slate-500">فیش حقوق و دستمزد</div><h2 className="mt-1 text-xl font-black">{company?.name || p.company_name || "شرکت"}</h2></div>
    ...
    <div className="grid grid-cols-2 gap-3 border-b border-slate-200 py-4 text-xs md:grid-cols-4">
      {[['نام', p.full_name], ['کد پرسنلی', p.personnel_code], ['کد ملی', p.national_id], ['عنوان شغلی', p.job_title || p.employee_job_title]].map(([a,v]) => <div key={a}><span className="text-slate-500">{a}</span><strong className="mt-1 block">{v || "—"}</strong></div>)}
    </div>
    <div className="grid grid-cols-2 gap-3 py-5 text-xs md:grid-cols-3">
      {[['حقوق پایه',p.base_salary],['مأموریت',p.mission_allowance],['پایه سنوات',p.seniority_allowance],['اضافه‌کاری',p.overtime],['پاداش',p.bonus],['سایر مزایا',p.other_benefits]].map(([a,v]) => <div key={a} className="rounded-2xl bg-slate-50 p-3">{a}<strong className="mt-1 block text-sm">{money(v)}</strong></div>)}
    </div>
    <div className="grid grid-cols-2 gap-3 border-t border-slate-200 pt-4 text-xs md:grid-cols-5">
      <div>روز کارکرد<strong className="mt-1 block">{p.work_days || 0}</strong></div>
      <div>روز مأموریت<strong className="mt-1 block">{p.mission_days || 0}</strong></div>
      <div>ساعت مأموریت<strong className="mt-1 block">{p.mission_hours || 0}</strong></div>
      <div>بیمه<strong className="mt-1 block">{money(p.insurance)}</strong></div>
      <div>مالیات<strong className="mt-1 block">{money(p.tax)}</strong></div>
    </div>
...
export default function PayslipPrintPage() {
  ...
  async function loadCompanies(){
    ...
    try{const r=await fetch("/api/companies",{cache:"no-store"}),d=await r.json();if(!r.ok||!d.success)throw Error(d.error||"خطا در شرکت‌ها");setCompanies(Array.isArray(d.data)?d.data:[]);}catch(e){setError(e.message||"خطا در دریافت شرکت‌ها")}finally{setLoading(false)}
  }
  async function loadCompanyData(id){
    ...
    try{
      ...
      const q=new URLSearchParams(window.location.search).get("ids");const requested=idsFrom(q);const available=(sd.data||[]).filter(x=>requested.includes(Number(x.id))).map(x=>Number(x.id));
      if(requested.length&&available.length){setSelected(available);setPreview((sd.data||[]).filter(x=>available.includes(Number(x.id))));}
      else {setSelected([]);setPreview([]);}
