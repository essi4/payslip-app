          ...
          <div className="border-b border-slate-700 bg-slate-900 px-4 py-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {menuItems.map((item) => {
                const active = isActive(item);
                return (
                  <Link key={item.href} href={item.href} className={["flex items-center justify-center gap-2 rounded-xl px-2 py-2.5 text-xs font-bold transition", active ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white"].join(" ")}>
